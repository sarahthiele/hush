...
\textbf{As discussed in https://arxiv.org/pdf/0705.3272.pdf, there exists a ``transition" GW frequency at which the GW spectrum transitions from confusion limited to being able to detect individual sources. In the confusion-limited regime, greater than one binary exists per LISA frequency bin, creating a superposition of signals which are unable to be disentangled. There are multiple parameters which serve to shape the Galactic DWD foreground. As discussed in https://arxiv.org/pdf/1912.02200.pdf, assuming the evolution of of each DWD system is driven solely by GW emission, the frequency derivative scales proportionally to $f_{\rm{GW}}^{11/3}$, causing a pileup of DWDs at lower frequencies. The distance-dependence of the spatial density of DWDs in the Galaxy also impacts the foreground amplitude, since strain scales inversely with distance. Star formation history assumptions impact the foreground height, as the duration of GW orbital evolution of each system will impact contributions to strain.}
